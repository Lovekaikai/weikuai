const verify = {
  getFields(nodeField, formData, isSubform) {
    let arr = []
    if (isSubform) {
      _(nodeField).forEach((res) => {
        const rule = this.getRuleObj(res, formData)
        arr.push(rule)
      })
    } else {
      _(nodeField).forEach((array) => {
        _(array).forEach((res) => {
          const rule = this.getRuleObj(res, formData)
          arr.push(rule)
        })
  ...
  getRuleObj(res, formData) {
    let rule = null
    if (res.default.field_auth != 40 && res.default.is_needed && res.default.type == 10) {
      let obj = {
        title: res.default.title,
        value: formData[res.default.name]
      }
      if (res.input.write_type == 20) {        // 数量验证
        obj.type = 'number'
      } else {
        obj.type = 'require'
      }
      rule = obj
    } else if (res.default.field_auth != 40 && res.default.is_needed && (res.default.type == 30 || res.default.type == 40 || res.default.type == 50 || res.default.type == 60 || res.default.type == 70 || res.default.type == 80 || res.default.type == 90 || res.default.type == 140 || res.default.type == 150 || res.default.type == 160)) {
      let obj = {
        type: 'require',
        title: res.default.title,
        value: formData[res.default.name]
  ...
  validateForm(fields) {
    ...
    for (var i = 0; i < fields.length; i++) {
      if (fields[i]) {
        if (!this.validateField(fields[i].type, fields[i].value)) {
          _g.openToast(fields[i].title + '不能为空')
          pass = false
          break
        }
      }
  ...
  validateField(type, val) {
    ...
    switch (type) {
      case 'require':
        result = this.isNotEmpty(val)
        break
      case 'number':
        result = this.isNumber(val)
    }
    return result
  },
  isNotEmpty(value) {
